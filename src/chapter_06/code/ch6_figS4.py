...
for i, a in enumerate(ax):
    ...
    phd.viz.titlebox(a, r'%s $\Delta \varepsilon_{RA}= %0.1f\ k_BT$' % (
        op_names[i], ops[i]), color=_['black'], pad=0.05, boxsize='12%',
        bgcolor='white', size=8)
    a.text(-0.32, 1.08, fig_labels[i], transform=a.transAxes, fontsize=8)
...
plt.tight_layout()
plt.savefig('../figs/ch6_figS4.pdf', bbox_inches='tight')
plt.savefig('../figs/ch6_figS4.png', bbox_inches='tight')

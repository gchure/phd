...
2e^{-\beta\Delta\varepsilon_{AI} }\left(} }{K_A^\text{(mut)} }^2}\left(K_A^\text{(mut)}
- K_I^\text{(mut)}\right)\left(c + K_I^\text{(mut)}\right) \over \left(c +
...
} }{K_A^\text{(wt)} }^2}\left(K_A^\text{(wt)} - K_I^\text{(wt)}\right)\left(c +
K_I^\text{(wt)}\right) \over \left(c + K_A^\text{(wt)}\right) \left[\left(c +
...
Middle column shows the allosteric contribution of free energy $F$ plotted as a
function of the inducer concentration. Right column shows the free energy
...
and $K_I$ change relative to wild-type, but maintain the same ratio. $\theta$ is
the scaling factor for both inducer dissociation constants. (B) Behavior of $F$
and $\Delta FF$ when the values of $K_A$ and $K_I$ change relative to the
wild-type, but by diffferent factors. IN both panels, the wild-type parameter
values were taken to be $K_A = 200\,\mu$M, $K_I = 1\,\mu$M and
$\Delta\varepsilon_{AI} = 4.5\,k_BT$. An interactive version of this figure is
available on the [paper
website](http://rpgroup.caltech.edu/mwc_mutants)](ch7_figS1){#fig:nonmono
short-caption="Non-monotonic behavior of $\Delta F$ with changes in $K_A$ and $K_I$."}

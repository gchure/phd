...
ppc_data = pd.read_csv('../../data/ch7_mutants_si/IND_prior_predictive_checks.csv')
...
for i, a in enumerate([ax5, ax6]):
    phd.viz.titlebox(a, models[i], size=8, bgcolor='white', color=colors['black'],
                     pad=0.05, boxsize='12%')
...
ax1.plot(k_range, ka_pdf, '-', color=colors['purple'], lw=0.75)
ax1.fill_between(k_range, ka_pdf, '-', color=colors['light_purple'], alpha=0.5)
ax2.plot(k_range, ki_pdf, '-', color=colors['purple'], lw=0.75)
ax2.fill_between(k_range, ki_pdf, '-', color=colors['light_purple'], alpha=0.5)
ax3.plot(ep_range, epAI_pdf, '-', color=colors['orange'], lw=0.74)
ax3.fill_between(ep_range, epAI_pdf, '-', color=colors['light_orange'], alpha=0.5)
ax4.plot(sig_range, sig_pdf, '-', color=colors['orange'], lw=0.75)
ax4.fill_between(sig_range, sig_pdf, '-', color=colors['light_orange'], alpha=0.5)
...
cmap_kakionly = {p:c for p, c in zip(percs, sns.color_palette('Purples', len(percs)))}
cmap_kakiepai = {p:c for p, c, in zip(percs, sns.color_palette('Oranges', len(percs)))}
...
plt.savefig('../figs/ch7_figS13.pdf',  bbox_inches='tight')
plt.savefig('../figs/ch7_figS13.png',  bbox_inches='tight')

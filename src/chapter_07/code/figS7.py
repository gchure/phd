...
data = pd.read_csv('../../data/ch7_mutants_si/empirical_F_prior_predictive_checks.csv')
...
# PRIOR DISTRIBUTIONS
# ##############################################################################
ax1.plot(x, fc_mu, '-', color=colors['purple'], lw=0.75)
ax2.plot(x, fc_sigma, '-', color=colors['purple'], lw=0.75)
ax2.fill_between(x, fc_sigma, '-', color=colors['light_purple'], alpha=0.5)
ax1.fill_between(x, fc_mu, color=colors['light_purple'], alpha=0.5)
...
perc_cmap = sns.color_palette('Purples', n_colors=(len(percs) + 2))
...
plt.tight_layout()
plt.savefig('../figs/ch7_figS7.pdf', bbox_inches='tight')
plt.savefig('../figs/ch7_figS7.png', bbox_inches='tight')

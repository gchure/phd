...
phd.viz.despine(ax)
fig.text(0.03, 0.95, '(A)', fontsize=8)
...
for i in range(2):
    ...
    _ = ax[0].plot(beta0_range, beta0_kde / beta0_kde.sum(), color=colors[color_key[i]],
                   lw=0.75, zorder=i+1, label=labels[i])
...
_ = ax[0].legend(fontsize=6, handlelength=1, loc='top left')
plt.tight_layout()
plt.savefig('../figs/ch9_figS9.png', bbox_inches='tight')
plt.savefig('../figs/ch9_figS9.pdf', bbox_inches='tight')
